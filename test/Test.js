...
contract('TrillioHeirs Contract', ([owner, alice_1, bob_2, carol_3, carlos, chad, mallory, ...rest]) => {
  ...
  describe('owner', async () => {
    ...
    //   const txInfo = await trillioHeirs.withdrawAll({ from: owner })
    //   const tx = await web3.eth.getTransaction(txInfo.tx);
    //   const gasCost = new BN(tx.gasPrice).mul(new BN(txInfo.receipt.gasUsed));
    //   expect((await web3.eth.getBalance(owner)).toString()).to.eql(new BN(balance).add(ether('1.5').add(gasCost)).toString())
    // })
  })
  ...
  describe('all counts', async () => {
    it('should mint 8888 only', async () => {
      ...
      for (const [index, item] of whitelist_2.entries()) {
        if (index >= 100)
          break
        const tx = await trillioHeirs.presaleMint(10, 1, tree_2.getHexProof(keccak256(item)), { from: item, value: web3.utils.toWei('1.5', 'ether') })
        for (const e of tx.logs)
          events.push(e.args.tokenId.toNumber())
      }
      ...
      for (const [index, item] of whitelist_3.entries()) {
        if (index >= 20)
          break
        const tx = await trillioHeirs.presaleMint(10, 1, tree_3.getHexProof(keccak256(item)), { from: item, value: web3.utils.toWei('1.5', 'ether') })
        for (const e of tx.logs)
          events.push(e.args.tokenId.toNumber())
      }
