...
contract ZamStacking is ReentrancyGuard, Initializable{
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                transferReward(address(rewardToken), msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function transferReward(address _token, address _to, uint _amount) internal {
        uint fee_transfer = 0;
        if (feeReceiver != address(0)) {
            uint fee_transfer = (_amount * fee) / FEE_DIVISOR;
